...
public class UserDAO {
	...
	public List<User> getItems(int offset, int row_count) {
		String sql = "SELECT * FROM user  order by user.id_user desc limit ?,?";
	...
	public int editItem(User objUser) {
		String sql = "update user set  username=?, password=?, fullname=?, email=?, role_id=?, picture=?, date_create=? where id_user=? ";
	...
	public int addItem(User objUser) {
		String sql = "insert into user	(username, password, fullname, enable, email, role_id, picture) values (?,?,?,?,?,?,?)";
	...
	public int active(Timestamp date, int idUser) {
		String sql = "update user set enable=1,date_create=? where user.id_user=? ";
		return jdbcTemplate.update(sql, new Object[] { date, idUser });
	}

	public int block(Timestamp date, int idUser) {
		String sql = "update user set enable=0,date_create=? where user.id_user=? ";
		return jdbcTemplate.update(sql, new Object[] { date, idUser });
	}
