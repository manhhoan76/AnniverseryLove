    ...
    <%@include file="/templates/taglib.jsp" %>
      ...
      <section id="main-content">
		  ...
		  <div class="row">
				...
				<div style="margin-left: 16px; margin-top: -14px; margin-bottom: 5px; display: inline;">
                      ...
                      <form class="navbar-form" style="display: inline-flex;" action="${pageContext.request.contextPath }/admin/cat/search">
                           <div>
                            <input class="form-control" placeholder="Search" type="text" name="key">
                      </div>
                        </form>
                       <a class="btn btn-success btn-sm"  href="${pageContext.request.contextPath }/admin/cat/add" title="Bootstrap 3 themes generator" >Thêm</a>
				</div>
			</div>
			<c:choose>
				<c:when test="${msg == 1 }">
				...
				<c:when test="${msg == 8}">
					<div class="panel-body"><div class="alert alert-danger fade in"><button data-dismiss="alert" class="close close-sm" type="button"><i class="icon-remove"></i></button><strong>Warning!</strong> Tên Danh mục đã tồn tại!</div></div>
				</c:when>
			</c:choose>
              <!-- page start-->                        
              <div class="row">
                      ...
                      <section class="panel">
                          <table class="table table-striped table-advance table-hover">
                           <tbody>
                              <tr>
								<th><i class=""></i>ID</th>
                                 <th><i class="icon_calendar"></i> Ảnh</th>
                              ...
                              <tr>
								 <td>${objCat.id }</td>
                                 <td><img alt="${objCat.name }" class="img-rounded" style="width: 127px; height: 89px;" src="${pageContext.request.contextPath }/files/${objCat.name }" /></td>
                                 <td>
                                  <div class="btn-group">
                                  <ul class="nav nav-tabs">
                                  	<li><a class="btn btn-primary" href="${pageContext.request.contextPath }/admin/cat/edit/${objCat.id }"><i class="icon_pencil-edit_alt"></i></a></li>
                                	<li><a class="btn btn-danger" onclick="return confirm('Bạn có muốn xóa không')" href="${pageContext.request.contextPath }/admin/cat/del/${objCat.id}"><i class="icon_close_alt2"></i></a></li>
                                  </ul>
                                  </div>
                                  </td>
                              </tr>
      ...
      <div class="pagination" style="margin-left: 23px;   margin-top: -31px;">           
			<div align="center" class="numbers">
			 <c:set value="${pageContext.request.contextPath }/admin/cat/index/${page-1 }"  var="urlPre"></c:set>
                <c:set value="${pageContext.request.contextPath }/admin/cat/index/${page+1 }" var="urlNext"></c:set>
                   <c:if test="${(page-1) > 0 }">
                    <a href="${urlPre }"><<</a>
                    </c:if>
                    <c:choose>
                    	<c:when test="${sumPage < 4 }">
                    		<c:forEach begin="1"  end="${sumPage }" var="i">
                      	 		<a <c:if test="${i == page }">class=active</c:if> href="${pageContext.request.contextPath }/admin/cat/index/${i }">${i } <span class="sr-only"></span></a>
                      		</c:forEach>
                    	</c:when>
